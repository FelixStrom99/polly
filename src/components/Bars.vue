<template>
  <div id="result-followup-container">
    <div style="height: 20%">
  <h6>{{title}}</h6>
    </div>
    <div class="result-Boxes">
      <div class="bar" v-for="(item, key,i) in data" v-bind:key="key">
        ...
        <div id="answer-alternative-text">
          <p v-if="correct[i]>0" style="color: #41B853">{{key}}</p>
          <p v-if="correct[i]<1" style="vertical-align: middle">{{key}}</p>
        </div>
      </div>
    </div>
  </div>
...
<style scoped>
#result-followup-container{
  border: solid #EFA500 0.2vw;
  border-radius: 6px;
  width: 48% ;
  height: 50vh;
  font-size: 1.2vw;
